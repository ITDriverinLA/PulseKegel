...
function generateEnvConfig() {
  const envConfigPath = path.join("client", "lib", "env-config.generated.ts");
  const iosKey = process.env.EXPO_PUBLIC_REVENUECAT_IOS_KEY || "";
  const androidKey = process.env.EXPO_PUBLIC_REVENUECAT_ANDROID_KEY || "";

  console.log(
    `Env config check: iOS key length=${iosKey.length}, Android key length=${androidKey.length}`,
  );
...
export const GENERATED_ENV = {
  REVENUECAT_IOS_KEY: ${JSON.stringify(iosKey)},
  REVENUECAT_ANDROID_KEY: ${JSON.stringify(androidKey)},
} as const;
`;

  fs.writeFileSync(envConfigPath, content);
  console.log(`Wrote env config to ${envConfigPath}`);
}
...
async function startMetro(expoPublicDomain) {
  ...
  const expoPublicVars = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key.startsWith("EXPO_PUBLIC_")) {
      expoPublicVars[key] = value;
    }
  }
  console.log(
    "EXPO_PUBLIC_* env vars found:",
    Object.keys(expoPublicVars).map(
      (k) => `${k} (length: ${(expoPublicVars[k] || "").length})`,
    ),
  );
