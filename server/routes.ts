...
try {
  ...
  APP_VERSION =
    appJson?.expo?.extra?.minimumVersion ?? appJson?.expo?.version ?? "0.0.0";
} catch {
...
const BLOG_SLUGS_TTL_MS =
  parseInt(process.env.BLOG_SLUGS_TTL_MS ?? "", 10) || 5 * 60 * 1000;
...
${blogUrls}
  ...
  app.get(["/for-partners", "/for-partners/"], (_req, res) => {
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        ...
        // Epic D: platform-split activation funnel (unique devices per stage).
        const funnelByPlatformRaw = await db.execute<{
          platform: string;
          opens: number;
          onboarded: number;
          started: number;
      ...
      `);
        ...
        const avgSessionsPerWau =
          wauCount > 0
            ? Math.round((Number(sessionsInWau?.count ?? 0) / wauCount) * 10) /
              10
            : 0;
